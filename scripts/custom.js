...
class Background extends GameObject {
  ...
  render() { this.x -= float2int((this.speed + float2int(objects[3].score / 60)) * deltaTime); if (this.x <= 0) this.x += 1080; renderImage(images[3], new Vector4(this.x - 540, 960, 1080, 1920), 0); renderImage(images[3], new Vector4(this.x + 540, 960, 1080, 1920), 0); }
...
class HorizontalObject extends GameObject {
  ...
  update() { if (!pause) { this.transform.position.x -= float2int((this.speed + float2int(objects[3].score / 60)) * deltaTime); if (this.transform.position.x < -this.transform.size.x / 2) this.destroyed = true; } }
  lateUpdate() { if (!pause) this.render(); }
